...
@pytest.mark.parametrize(
    "cli_inputs,new_command,module_to_patch,paths_to_mocked_funcs",
    (
        (["build-image"], "image build", "image_commands", ["build.build"]),
        (
            ["run-job", "--direct-runner"],
            "job run",
            "job_commands",
            ["run.RunPipeline.return_value.run"],
        ),
        (
            ["stop-job"],
            "job stop",
            "job_commands",
            ["stop.StopJob.return_value.stop"],
        ),
        (
            ["deploy-job", "--direct-runner"],
            "job deploy",
            "job_commands",
            [
                "stop.StopJob.return_value.stop",
        ...
        (
            ...
            "job create",
            "job_commands",
            ["create.CreateJob.return_value.create"],
        ),
        (
            ["publish", "d34db33f"],
            "message publish",
        ...
        (
            ...
            "job test",
            "job_commands",
            ["test.TestPipeline.return_value.run"],
        ),
        (
            ["verify-job"],
            "job verify",
            "job_commands",
            ["verify.VerifyJob.return_value.verify_job"],
        ),
    ),
)
def test_aliased_commands(
    cli_inputs,
    new_command,
    module_to_patch,
    paths_to_mocked_funcs,
    runner,
    mocker,
    monkeypatch,
    mock_warn_if_py2_job,
):
    """Deprecated commands are aliased to the correct supported command."""
    ...
    monkeypatch.setattr(config_utils, "get_config_by_path", mock_config)

    mock_commands = mocker.Mock()
    monkeypatch.setattr(cli, module_to_patch, mock_commands)
    mock_funcs = []
    for path_to_mocked_func in paths_to_mocked_funcs:
        # recursively get the mocked func
        path = path_to_mocked_func.split(".")
        mock_func = mock_commands
        for p in path:
            mock_func = getattr(mock_func, p)

        # for those commands that return an exit code from the container
        mock_func.return_value = 0
        mock_funcs.append(mock_func)

    expected_warning = "Warning: command is deprecated. "
    expected_warning += "Please use `klio {}`.\n".format(new_command)

    result = runner.invoke(cli.main, cli_inputs)

    assert expected_warning == result.output
